...
class Components extends Component {
    ...
    render() {
        ...
        const handleShowSignUpDialog = () => {
            this.setState(estadoAtual => ({ showSignUpDialog: !estadoAtual.showSignUpDialog }));
        };

        const renderComponent = () => {
            const { NUMBER_LIST, NAME_FORM, TEMPERATURE_CALCULADOR, SIGNUP_DIALOG, FILTERABLE_PRODUCT_TABLE } = SAMPLES;
            switch (selectedSample) {
                ...
                case SIGNUP_DIALOG:
                    return (
                        <div>
                            <button onClick={handleShowSignUpDialog}>Quero me Cadastrar</button>
                            <SignUpDialog show={showSignUpDialog} id="dialog-signup" onClose={handleShowSignUpDialog} />
                        </div>
                    );
            ...
            <div style={pageStyles}>
                <fieldset>
                    ...
                    <select value={selectedSample} onChange={handleChangeSample}>
                        {Object.keys(SAMPLES).map((element, ix) => <option key={ix} value={SAMPLES[element]}>{SAMPLES[element]}</option>)}
                    </select>
                </fieldset>
                <br />
                <div>
